// Dog Face Line Art — NEW WIDGET (bypass cache)
console.log("Dog Line Art NEW WIDGET v1 - " + Date.now());

// Direct CORS requests only
const DDL_BASE = "https://dogfaceart2.onrender.com";

// Compress image client-side
async function compressImageIfNeeded(file) {
  const SAFE_BYTES = 900 * 1024;
  ...
  function initBlock(root) {
    ...
    async function callPreview() {
      ...
      try {
        ...
        const data = await makeRequest("/apps/dogart/preview", fd);
    ...
    async function approve() {
      ...
      try {
        const fd = new FormData(); 
        fd.append("finalB64", finalB64);
        const data = await makeRequest("/apps/dogart/finalize", fd);
    ...
    console.log("Dog Line Art NEW WIDGET: bound");
  }
